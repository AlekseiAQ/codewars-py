"""Sort by binary ones
https://www.codewars.com/kata/sort-by-binary-ones
...
def sort_by_binary_ones(num_list):
    def key_function(number):
        string_ = format(number, 'b')
        return -string_.count('1'), len(string_), number
    return sorted(num_list, key=key_function)
