...
https://www.codewars.com/kata/generating-numbers-from-digits-number-1
...
def proc_arr(arr: list) -> list:
    string_ = ''.join(sorted(arr))
    return [factorial(len(arr)) // reduce(
                mul, map(factorial, Counter(arr).values())),
            int(string_),
            int(string_[::-1])]
